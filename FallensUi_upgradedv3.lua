...
function Orionlib_ui_ui:BuildMainFrame()
    ...
    -- and set scroll canvas to full content height
    self.settings_panel = _real_settings_panel
    settings_scroll.CanvasSize = UDim2.new(0, 0, 0, rowY + 14 * scale_factor)
    ...
    local function setSettingsPanelOpen(openState)
        ...
        if self.settings_open then
            ...
            local contentH = rowY + 14 * scale_factor
            -- Clamp panel height: can't be taller than the space from top of main_frame
            -- to the button (position Y = 1 - 36px from bottom of main_frame)
            local maxH = contentH
            if self.main_frame and self.main_frame.Parent then
                local frameH = self.main_frame.AbsoluteSize.Y
                -- Panel bottom is at (frameH - 36px), panel grows upward
                -- So max height = (frameH - 36px - 8px top margin)
