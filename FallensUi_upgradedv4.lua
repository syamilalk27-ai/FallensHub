...
function Orionlib_ui_ui:BuildMainFrame()
    ...
    if logoDisplayWidth > 0 then
        ...
        local nameMaxW = math.max(80 * scale_factor, headerNameMaxWidth - logoDisplayWidth)
        local hubNameText = create("TextLabel", {
            ...
            Size = UDim2.new(0, nameMaxW, 0, 22 * scale_factor),
            TextTruncate = Enum.TextTruncate.AtEnd,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = self.main_frame
        })
    ...
    -- Parent trackFrame to screen_gui so position calc is accurate even when panel clips
    local _sb = attach_scrollbar(self, settings_scroll, _real_settings_panel, {
        TrackWidth = 5,
        ThumbWidth = 3,
        EdgeInset = 3,
        VerticalInset = 6,
    ...
    if _sb and _sb.Track then
        _sb.Track.Parent = self.screen_gui
    end
