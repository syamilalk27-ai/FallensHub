...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- subtle soft-light glow underneath the bar
        local GlowSpread = New("Frame", {
            Name = "GlowSpread",
            BackgroundColor3 = "AccentColor",
        ...
        Library.GlowBar = GlowBar

        local GlowPulseInfo = TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true)
        Library.GlowTween = TweenService:Create(GlowBar, GlowPulseInfo, {
            BackgroundTransparency = 0.05,
        })

        if Library.EnableGlowBar then
            Library.GlowTween:Play()
        else
            GlowBar.Visible = false
            GlowSpread.Visible = false
        end

        function Window:SetGlowBarEnabled(Enabled: boolean)
            Library.EnableGlowBar = Enabled
            GlowBar.Visible = Enabled
            GlowSpread.Visible = Enabled

            if Enabled then
                Library.GlowTween:Play()
            else
                Library.GlowTween:Pause()
            end
        end
...
function Library:Unload()
    ...
    if Library.GlowTween then
        Library.GlowTween:Cancel()
        Library.GlowTween:Destroy()
        Library.GlowTween = nil
    end
